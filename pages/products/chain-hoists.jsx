...
const ChainHoistsPage = ({data, page, category, maxPage}) => {

    return (
        <main>
            <Head>
              <title>Jual Chain Hoist di BJA Hoist | Melayani jasa service hoist crane dan system electrical hoist crane serta pemasangan instalasi electrical hoist crane</title>
              <meta name="description" content="BJA Hoist menjual chain hoist baru maupun second dengan kualitas terbaik. BJA Hoist / bjahoist (Bintang Jaya Abadi) melayani jasa service hoist crane dan system electrical hoist crane, pemasangan instalasi electrical hoist crane, pembuatan cargo lift / lift barang, serta penjualan hoist crane,chain hoist, wire rope hoist, dan bagian atau aksesoris hoist" />
            ...
            <ChainHoist data={data} category={category} />
            <Pagination maxPage={maxPage} page={page} link={`/products/${category}`} />
        </main>
    )
}

export default ChainHoistsPage

export async function getServerSideProps({query}) {
    try {
        let response = await axios.get(`/products/chain-hoists?page=${query.page ? query.page : 1}`)
